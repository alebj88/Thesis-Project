...
procesamientoDeImagenes <-function(
                            imagPNG,newDir=FALSE,pathPPD="",HT=TRUE,nSet=FALSE){
    #setwd("~/");imagPNG <-"pruebadem.png";newDir<-TRUE;newDir<-FALSE
    img <-readPNG(imagPNG)
    ...
    if(nSet == FALSE){  #Si la interpretacion ya se realizo no la repetimos.
        ...
        if(sum(is.na(numeros)) != 0 | Forzar == TRUE){
            ...
            cat("\n")
            stop("Ejecute el programa nuevamente para incluir los cambios.",
                 call.=FALSE)
