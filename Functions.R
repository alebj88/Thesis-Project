...
distGoogleEarth <-function(LatLonTB,tablaGE){
    ...
    timeCor <- unname(apply(LatLonTB,1,function(x){
        ...
        for(i in 1:7){
            #Procedimiento
            listaRec <- lapply(reco,function(r){
                marg <- 0.01*i
                zona <- tablaGE[tablaGE$latitude [r] <=LCCC$latitude[r] + marg &
                                tablaGE$latitude [r] >=LCCC$latitude[r] - marg &
                                tablaGE$longitude[r] <=LCCC$longitude[r]+ marg &
                                tablaGE$longitude[r] >=LCCC$longitude[r]- marg &
                                tablaGE$elect == FALSE,]
                #Asignamos ubicaciones e impedimos su reasignacion.
                if(nrow(zona) != 0){
                    zona$elect <- NULL
                    zona$tiempoAc <-tiempo[r]
                    zona$latitude  <- rep(LCCC$latitude[r] ,times=nrow(tablaGE))
                    zona$longitude <- rep(LCCC$longitude[r],times=nrow(tablaGE))
                    tablaGE$elect[
