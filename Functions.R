...
procesamientoDeImagenes <-function(imagPNG,newDir= FALSE,pathPPD = "",HT = TRUE,
    ...
    #imagPNG <-"NOBM_DAY_R2017_h_2014-05-21T00_00_00_overlay_frame.png";
    #setwd("~/");newDir<-TRUE;HT <-TRUE;mapCenter <- c(12.5,-66.5);nSet <- FALSE
    #pathPPD <- ""
    #Calculo de bordes.
    ...
    #Banda superior.
    superior<-bandaColores[10:12,12:14,]
    ...
    #Banda Inferior.
    inferior<-bandaColores[380:382,12:14,]
    ...
    if("depth.m" %in% loc == TRUE){
        ...
        for (np in 1:length(profundidades)){
            ...
            f_prodd <- sapply(imputedVars,function(impVar){
                ...
                #Check
                if(sum(is.na(datAux[,1]))*length(profundidades) != sum(is.na(dat[,1]))){
...
distGoogleEarth <- function(LatLonTB,tablaGE,dlMax=150,tiempoMax=14){
        ...
        for(i in 1:10){             #Originalmente 1:6.
            ...
            listaRec <- apply(dfReco,1,function(r){
                ...
                zona <- tablaGE[tablaGE$latitude  <= 
                        LCCC$latitude[ as.numeric(as.character(r[1]))] + m &
                                tablaGE$latitude  >=
                        LCCC$latitude[ as.numeric(as.character(r[1]))] - m &
                                tablaGE$longitude <= 
                        LCCC$longitude[as.numeric(as.character(r[1]))] + m &
                                tablaGE$longitude >= 
                        LCCC$longitude[as.numeric(as.character(r[1]))] - m &
                        tablaGE$elect == FALSE,]
                    ...
                    D <- apply(zona[c("latitude","longitude")],1,function(f){
                        dc <- distGL(
                            ...
                            c(LCCC$latitude[as.numeric(as.character(r[1]))], 
                              LCCC$longitude[as.numeric(as.character(r[1]))]))
                        return(dc)
                    })
                    D <- unname(D)
    ...
    dataTiempoRec <- dataTiempoRec[dataTiempoRec$valor <= tiempoMax,]
    ...
    if(nrow(tablaA) == 0){
        ...
        stop("Incremente la distancia o el tiempo de viaje en corriente.",call.= F)
    }
    
    #MODIFICACION FINAL.------------------- (Seccion borrable)
    tablaA1 <- tablaA[tablaA$tipo == "Tiempo de viaje en corriente (Dias)",] 
    tablaA1$tipo <- NULL
    names(tablaA1)[-c(1,2)] <- paste0(names(tablaA1)[-c(1,2)],".t")
    tablaA1$clave <- paste(tablaA1$latitude,tablaA1$longitude,sep="_") 
    
    tablaA2 <- tablaA[tablaA$tipo == "Distancia Lineal (Kms)",]
    tablaA2$tipo <- NULL
    names(tablaA2)[-c(1,2)] <- paste0(names(tablaA2)[-c(1,2)],".d")
    tablaA2$clave <- paste(tablaA2$latitude,tablaA2$longitude,sep="_") 
    ...
    #Merge.
    tablaA <- merge(tablaA1[,-c(1,2)],tablaA2[,-c(1,2)],by="clave",all=TRUE)
    tablaA <- separate(tablaA,col=clave,into=c("latitude","longitude"),sep="_")
    
    #Renombramos.
    names(tablaA)[which(names(tablaA) =="valor.t")] <-"Tiempo.en.Corriente.dias"
    names(tablaA)[which(names(tablaA) =="valor.d")] <-"Distancia.Euclidea.kms"
    
    #Formateamos
    tablaA$latitude <- as.numeric(as.character(tablaA$latitude))
    tablaA$longitude <- as.numeric(as.character(tablaA$longitude))
    
    #Eliminamos negativos.... Hay algo raro por ahi
    tablaA$Tiempo.en.Corriente.dias[(tablaA$Tiempo.en.Corriente.dias < 0) &
...
impute.per.regionAux <- function(df1,radio=1,medida="median"){    #Radio en kms.
    ...
    salida <-cbind(df1[,which(names(df1) %in% c("latitude","longitude"))],salida)
    return(salida)
}
