...
#Si el dato no es NA, NO HALLARLE SUS VECINOS ES INUTIL, poner vecinos en NA.
#SI un dato no tien vecinos retornar el dato NO NA. Es inutil igualmente. Basura Sos basura.
...
impute.per.region2 <- function(dat,imputador,radio=c(1,5,10),
    ...
    if("depth.m" %in% loc == TRUE){
        ...
        #Merge
        dat <- merge(
            dat,imputador[c("depth.m",imputedVars,"clave")],by="clave",all.x=T)
    }else{
        #Claves
        dat$clave <- paste(dat$latitude,dat$longitude,dat$depth.m,sep="_")
    ...
    #Check
    if(identical(names(imputador),names(dat)) == FALSE){
        stop("Las variables de los dataset imputador y dat no coinciden.")
    }
    ...
    if(any(!is.na(dat$clave[check2 %in% check]))){
        stop("El vector dat$clave presenta NA's en posiciones no esperadas.")
    }
    if(any(is.na(dat$clave[!(check2 %in% check)]))){
        stop("El vector dat$clave tiene valores no nulos en posiciones no esperadas.")
    }
    
    #liberamos memoria
    rm(imputador);rm(check);rm(check2)
    gc()
       ...
       #                      (!is.na(datAux[,1]) & !is.na(datAux[,4])),]
        ...
        f_prodd <- sapply(imputedVars,function(impVar){
            ...
            datAux <- datAux[is.na(datAux[,1]) | 
                                 (!is.na(datAux[,1]) & !is.na(datAux[,4])),]
            ##Establecemos rango.
            rango <- c(nrow(dat)-sum(is.na(dat$clave))+1,nrow(dat))
    ...
    #El data frame de salida se llama dat.
    ...
    if(identical(unique(claveDat),unique(claveDatos)) == FALSE){
        stop("Las filas del dataset imputado no estan en el orden esperado.")
    }
