...
#Recibe un data frame con lat, lon y las variables a imputar (data).
#Retorna un data frame con lat, lon y las variables imputadas.
impute.per.region <- function(data,radio=1,medida="median",rango=0,varNro=0){
    
    #Extraemos la region de busqueda y variables de correccion.
    RegionB <- data[names(data) %in% c("latitude","longitude")]
    varRB   <- data[!(names(data) %in% c("latitude","longitude"))]
    
    #Extraemos la region a corregir.
    RegionC <- RegionB
    varRC   <- varRB
    if(length(rango) != 1){     #Una seccion
        RegionC <- RegionC[rango[1]:rango[2],]
        varRC   <- varRC[rango[1]:rango[2],]
        if(class(varRC) != "data.frame"){
            varRC   <- as.data.frame(varRC)
            names(varRC) <- names(data)[
                                !(names(data) %in% c("latitude","longitude"))]
        }
    }
    #Recortamos el conjunto de variables que se van a trabajar.
    if(varNro != 0){           #Si es solo una variable.
        varRB <- varRB[names(data)[varNro]]
        varRC <- varRC[names(data)[varNro]]
    }
    
    #Extraemos las posiciones a eliminar de RegionB y varRB.
    elim <- unname(apply(varRB,1,function(f){
        bool <- any(!is.na(f)) 
        return(bool)
    }))
    
    #Eliminamos de RegionB las filas cuyas variables solo contengan NA's.
    RegionB <- RegionB[elim,]
    
    #Eliminamos de varRB las mismas filas.
    varRB <- varRB[elim,]
    if(class(varRB) != "data.frame"){
        varRB <- as.data.frame(varRB)
        names(varRB) <- names(varRC)
    }
    
    #Damos formato numeric a lat-lon.
    RegionC[,1] <-as.numeric(as.character(RegionC[,1]))   
    RegionC[,2] <-as.numeric(as.character(RegionC[,2]))
    RegionB[,1] <-as.numeric(as.character(RegionB[,1])) 
    RegionB[,2] <-as.numeric(as.character(RegionB[,2]))
    ...
    #Incluimos informacion sobre el tipo de variable en el dataset varRC.                       
    clases <-unname(sapply(varRC,class))
    varRC[2:(nrow(varRC)+1),] <- varRC 
    varRC[1,] <- clases
    
    #Incluimos informacion sobre el nombre de la variable en el dataset varRC.
    varRC[2:(nrow(varRC)+1),] <- varRC 
    varRC[1,] <- names(varRC)
    
    #Asignamos la clase correcta a las variables del dataset y procesamos.
    correctedVars <-apply(varRC,2,function(x){
        #Extraemos variable de imputacion.
        varRBemp <- varRB[x[1]]
        #Eliminamos valor
        x <- x[-1]
        #Procesamos
        ...
        }else if(x[1] == "numeric" | x[1] == "integer"){
            x <-as.numeric(as.character(x[-1]))  #Tipo real de variable.
        }else{
            stop("La clase del la variable a imputar no fue definida.")
        }
        x <-data.frame(varX=x,sec=(1:length(x)))
        z <-apply(x,1,function(y){
                ...
                if(is.na(y[1])){     #Intentamos imputar si es NA.
                    ...
                    ubic <-RegionB$clave %in% vecinos[[as.numeric(y[2])]]
                    valores <-varRBemp[ubic,]
                    if(class(valores)=="character" | class(valores)=="factor"){  
                        t <-table(valores)
                    ...
                    if(medida == "median"){           #Mediana.
                        ...
                        if(all(is.na(valores))){  #Vecinos vacios.
                            warning("varRB tiene NA's")
                            y[1] <-NA
                        }else{
                            #Geolocalizacion del NA de varRC.
                            latNA <-RegionC$latitude[as.numeric(y[2])]
                            lonNA <-RegionC$longitude[as.numeric(y[2])]
                            
                            #Geolocalizacion de los vecinos del NA en RegionB.
                            geoVeci <- RegionB[ubic,
                                 names(RegionB) %in%  c("latitude","longitude")]
                            
                            #Removemos geolocalizaciones sin datos.
                            if(sum(is.na(valores)) != 0){
                                warning("varRB tiene NA's") 
                                geoVeci <-geoVeci[!is.na(valores),]
    ...
    #Transformamos en data frame la lista 'correctedVars'.
    correctedVars <-as.data.frame(correctedVars) 

    #Recuperamos la tabla varRC original
    varRC <- as.data.frame(varRC[-c(1,2),])
    names(varRC) <- names(varRB)
    
    #Le damos a 'correctedVars' el formato original.
    for(i in 1:ncol(correctedVars)){        #Formato adecuado de correctedVars.
        if(clases[i] == "numeric" | clases[i] == "integer"){
            correctedVars[,i] <- as.numeric(as.character(correctedVars[,i]))
        }else{
            correctedVars[,i] <- as.character(correctedVars[,i])
        }
    }
    names(correctedVars) <- names(varRC)
    View(correctedVars)
    #Control de salida
    if(ncol(correctedVars) != ncol(varRC)){
        stop("El numero de columnas de correctedVars no coincide con el de varRC.")
    }
    if(nrow(correctedVars) != nrow(varRC)){
        stop("El numero de filas de correctedVars no coincide con el de varRC.")
    }
    if(length(rango) == 1 & nrow(correctedVars) != nrow(data)){         
        stop("El numero de filas de correctedVars no coincide con el de data. (rango=0)")
    }
    if(varNro == 0 & ncol(correctedVars) != (ncol(data)-2)){ 
        stop("El numero de columnas de correctedVars no coincide con el de data-2. (varNro=0)")
    }
    if(varNro != 0 & ncol(correctedVars) != 1){ 
        stop("El numero de columnas de correctedVars no es 1 y varNro != 0")
    }
    if(length(rango) != 1 & nrow(correctedVars) != (rango[2] - rango[1] + 1)){
        stop("El dataset 'correctedVars' tiene una cantidad incorrecta de filas.")
    }
    
    #Agregamos los datos nuevos al dataset original
    #Todas las filas, todas las variables.
    if(length(rango) == 1 & varNro == 0){         
        data <- cbind(data[c("latitude","longitude")],correctedVars)
    #Todas las filas, una variable.
    }else if(length(rango) == 1 & varNro != 0){ 
        data[,varNro] <- correctedVars 
    #Seccion de filas, todas las variables.
    }else if(length(rango) != 1 & varNro == 0){
        data[rango[1]:rango[2],] <- cbind(
                data[rango[1]:rango[2],c("latitude","longitude")],correctedVars)
    #Seccion de filas, una variable.
    }else{
        data[rango[1]:rango[2],varNro] <- correctedVars
    }
    
    #Aseguramos que la salida sea un data frame.
    if(class(data) != "data.frame"){
        data <- as.data.frame(data)
        names(data) <- c("latitude","longitude",names(correctedVars))
    }
...
impute.per.region2 <- function(dat,imputador,radio=c(1,5,10),
                                                    medida="median",varName=0){
    #Data original
    datos <- dat
    ...
    if(varName != 0){
        if(varName %in% names(imputador) == TRUE){
            imputador <- imputador[c("latitude","longitude",varName)] 
            imputedVars <- varName
        }else{
            warning("La variable '",varName,"' no pertenece al conjunto de datos. 
            Conjunto completo procesado.")   
            imputedVars <- names(imputador)[
                !(names(imputador) %in% c("latitude","longitude"))]
        }
    }else{
        imputedVars <- names(imputador)[
            !(names(imputador) %in% c("latitude","longitude"))]
    ...
    #Merge
    dat <- merge(dat,imputador[,
        !(names(imputador) %in% c("latitude","longitude"))],by="clave",all.x=T)
    
    #Valor de clave que permite identificar el dataset original.
    dat$clave <- rep(NA,times=nrow(dat))
    
    #Check
    if(length(names(imputador)) != length(names(dat))){
        stop("Los dataset imputador y dat tienen distinto numero de variables")
    }
    ...
    #Colocamos la variable clave en la posicion 1.
    posClave <- which(names(dat) == "clave")
    resto <- which(names(dat) != "clave")
    dat <- dat[,c(posClave,resto)]
    
    #Establecemos el rango de imputacion del dataset 'dat'.
    rango <- c(nrow(imputador)+1,nrow(dat))

    #Generamos lista con las variables imputadas
    dataList <- list()
    for(i in 1:length(radio)){
        dfAux <- impute.per.region(dat[,-1],
                                   radio=radio[i],medida=medida,rango=rango)
        dfAux$clave <- dat[,1]
        dfAux <- dfAux[imputedVars][is.na(dfAux$clave),]
        if(length(imputedVars) == 1){
            dfAux <- as.data.frame(dfAux)
            names(dfAux) <- imputedVars
        }
        dataList[[length(dataList)+1]] <- dfAux
    }
    
    #Recuperamos la tabla con el formato correcto de salida.
    dat <- dat[is.na(dat$clave),]
    dat$clave <- NULL
    
    #Corregimos los NA's. del dataset de salida.
    #lapply funcionara como un procedimiento no como una funcion.
    l <- lapply(dataList,function(df){
        for (i in 1:length(imputedVars)){
            var <- imputedVars[i]
            dat[var][is.na(dat[var][,1]),] <<- df[var][is.na(dat[var][,1]),]
        }
    })
    
    #Check
    if(nrow(dat) != nrow(datos)){
        stop("El numero de filas del dataset imputado no coincide con el original.")
    }
    
    datos <- cbind(datos,dat[imputedVars])
    return(datos)
}
