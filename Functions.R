...
procesamientoDeImagenes<-function(imagPNG){
    setwd("~/");imagPNG <-"MonthlyandSeasonalAverages2.png"
    img <-readPNG(imagPNG)
    ...
    setwd("~/Thesis Project/Data/Raw Data/Giovanni/Ctrl")
    png(file=paste0(nombre,"NRO.png"))
    plot(c(1,1124),c(1,867),type='n',main=paste0("NRO ",nombre))         
    ...
    if(sum(is.na(numeros)) != 0){
        print(numeros)
        stop("La imagen no pudo ser procesada correctamente. El interpretador de
             caracteres requiere ajuste. NA's detectados.")
    }    
    if(sum(diff(rev(numeros)) <= 0) != 0){
        print(numeros)
        stop("La imagen no pudo ser procesada correctamente. El interpretador de
             caracteres requiere ajuste. Escala incorrecta.")
    }    
    ...
    row.names(mapaNum) <-latitude
    colnames(mapaNum) <-longitude
    
    #Transformamos mapa numerico en dataset para su exportacion.

    #archivo<-data.frame(latitude=latitude,longitude=longitude)
    #VALIDAR TODO ESTOOOOOOOOOOOOO
