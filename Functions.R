...
procesamientoDeImagenes<-function(imagPNG,newDir=FALSE,pathPPD="",HT=TRUE){
    ...
    #setwd("~/");imagPNG <-"MODISA_L3m_PIC_2014_pic_2015-12-01T00_25_10_overlay_frame.png"
    #setwd("~/");imagPNG <-"MODISA_L3m_PIC_2014_pic_2015-11-01T00_10_10_overlay_frame.png"
    #setwd("~/");imagPNG <-"SeaWiFS_L3m_IOP_2014_aph_443_giop_2010-09-30T19_04_26_overlay_frame.png"
    #setwd("~/");imagPNG <-"M2TMNXADG_5_12_4_DUDP001_2015-12-01T00_00_00_overlay_frame.png"
    img <-readPNG(imagPNG)
    ...
    if(newDir==FALSE){
        ...
        suma <-apply(img[,1:100,1],1,sum)
        suma[suma<100] <-0
        corteInf <-dim(img)[1]-which.min(rev(suma))+1
        corteSup <-corteInf - 530 
    ...
    if(newDir==FALSE){
        ...
        prueba1<-identical(as.numeric(dim(img)),c(531,1024,4))
    }
    #prueba1<-TRUE
    if(prueba1 == FALSE){
...
#Calculo de distancia entre puntos de geolocalizacion.   
distGL <-function(p1,p2,r=6371){
    ...
    if(abs(value - 1) < 0.000001){ #Esto es un Bug letal, un super bug.
        alp <-0
    }else{
        alp <-acos(value)
    }
...
#Algoritmo para imputar datos por cercania geografica.
imputVectorGL <-function(GLdf,vect,r=1,medida="median"){
    if(nrow(GLdf) != length(vect)){
        stop("El data frame de geolocalizacion y la variable a imputar deben tener la misma longitud.")
    }
    if(ncol(GLdf) != 2){
        stop("El data frame de geolocalizacion debe tener solo dos columnas.")
    }
    df<-cbind(GLdf,vect)
    valores <-unname(apply(df,1,function(x){
                      vecinos <-df[,3][which(unname(apply(df[,-3],1,function(y){
                                    distGL(c(y[1],y[2]),c(x[1],x[2]))
                            })) <= r)]
                      if(medida == "median"){
                          salida <-median(vecinos,na.rm=TRUE)
                      }else if(medida == "mean"){
                          salida <-mean(vecinos,na.rm=TRUE)
                      }
                      return(salida)
              }))
    vect[is.na(vect)] <-valores[is.na(vect)]
    return(vect)
}  
