...
impute.per.region <- function(data,radio=1,medida="median",rango=0){
    ...
    correctedVar <-unname(apply(RegionC,1,function(y){
             ...
             if(is.na(y[3])){      #Intentamos imputar si es NA.
                ...
                if(is.na(vecinos[[y[4]]])){
                    return(y[3])            
                }
                #Buscamos los valores de correccion.
                ubic <- RegionB$clave %in% vecinos[[y[4]]]
                ...
                if(medida == "median"){                      #Mediana.
                    ...
                    latNA <-RegionC$latitude[y[4]]
                    lonNA <-RegionC$longitude[y[4]]
...
distGoogleEarth <-function(LatLonTB,tablaGE){
    #Reducimos el trabajo computacional.
    LatLonTB <- LatLonTB %>% unique()
    
    #Agregamos clave a tablaGE y variable de control
    tablaGE$clave <- paste0(tablaGE[c("latitude","longitude")])
    tablaGE$elect <- rep(FALSE,times=nrow(tablaGE))
    
    #Agregamos a tablaGE el tiempo de de viaje.
    tablaGE$tiempo.dias <- rep(0,times=nrow(tablaGE))
    
    #Distancia lineal----------------------------------
    distLin <- unname(apply(LatLonTB,1,function(x){
        dist2 <-unname(apply(tablaGE[c("latitude","longitude")],1,function(y){
            vect <- distGL(c(x[1],x[2]),c(y[1],y[2]))
        }))
        objt <- tablaGE$objeto[dist2 <= 200]
        dist2 <- dist2[dist2 <= 200]
        #Check
        if(length(objt) != length(dist2)){
            stop("Las longitudes de objt y de dist2 no coinciden.")
        }
        med <- rep("Distancia Lineal (Kms)",times=length(objt))
        lat <- rep(x[1],times=length(objt))
        lon <- rep(x[2],times=length(objt))
        datosPtoi <- data.frame(latitude = lat,longitude = lon,objeto = objt,
                                medida = med,valor = dist2)
        return(datosPtoi)
    }))
    #Conseguimos un data frame con latitude, longitud, objeto, medida y valor 
    #para cada fila de la tabla LatLonTB. Estos valores de latitud y longitud
    #coinciden con los vistos en la tabla ambiental Google Earth (tablaGE).
    ...
    timeCor <- unname(apply(LatLonTB,1,function(x){
        ...
        posMin <-puntosCaja$posicion[which.min(dist)] #Posicion en la costa
        ...
        if(1 <= posMin & posMin <= 45){          #Del 1 al  19 =  RE
            reco <- posMin:1
        }else if(46 <= posMin & posMin <= 87){   #Del 46 al  87 =  RP
            reco <- posMin:87
        }else if(88 <= posMin & posMin <= 95){   #Del 88 al  95 =  RE
            reco <- posMin:87
        }else if(96 <= posMin & posMin <= 171){  #Del 96 al 171 =  RP
            reco <- posMin:171
        }else if(172 <= posMin & posMin <= 177){ #Del 172 al 177 =  RE
            reco <- posMin:171
        }else if(178 <= posMin & posMin <= 200){ #Del 178 al 200 =  RP
            reco <- posMin:200
        }else if(201 <= posMin & posMin <= 210){ #Del 201 al 210 =  RE
            reco <- posMin:200
        }else if(211 <= posMin & posMin <= 215){ #Del 211 al 215 =  RP
            reco <- posMin:215
        }else if(216 <= posMin & posMin <= 218){ #Del 216 al 218 =  RE
            reco <- posMin:215   
        }else if(219 <= posMin & posMin <= 255){ #Del 219 al 255 =  RP
            reco <- posMin:255 
        }else{
            stop("posMin fuera de rango.")
        }
        #Vemos en que direccion avanza.
        if(reco[1] >= reco[length(reco)]){       #Direccion al predecesor.
            tiempos <- c(0,LCCC$tiempoViaje.Adj.dias[reco][-length(reco)])
        }else{                                   #Direccion al sucesor.
            tiempos <- c(0,LCCC$tiempoViaje.Adj.dias[reco][-1])
        }
        tiempos <- cumsum(tiempos) #Tiempo acumulado de recorrido.
        
        #Limitamos la busqueda a dos semanas de viaje.
        reco <- reco[tiempo <= 14] 
        tiempo <- tiempo[tiempo <= 14] 
        
        #Lo volvemos data frame.
        reco <- data.frame(reco=reco,tiempo=tiempo,pos=seq(1:length(reco)))
        
        #Ciclo de busqueda de objetos costeros a lo largo de la ruta.
        listaDF <- list()
        for(i in 1:7){
            #Procedimiento
            listaRec <- apply(reco,1,function(r){
                m <- 0.01*i
                zona <- tablaGE[tablaGE$latitude  <= LCCC$latitude[r[1]] + m &
                                tablaGE$latitude  >= LCCC$latitude[r[1]] - m &
                                tablaGE$longitude <= LCCC$longitude[r[1]]+ m &
                                tablaGE$longitude >= LCCC$longitude[r[1]]- m &
                                tablaGE$elect == FALSE,]
                zona$posicion <- 1:nrow(zona)
                if(identical(names(zona),c("latitude","longitude","clave",
                                   "elect","tiempo.dias","posicion")) == FALSE){
                    stop("El dataset zona esta desordenado.")
                }
                #Asignamos ubicaciones e impedimos su reasignacion.
                if(nrow(zona) != 0){
                    posAct <- r[1]       #Posicion de recorrido en la tabla LCCC
                    if(r[3] == nrow(reco)){          #Si estamos en la ultima
                        posSig <-reco$reco[r[3] - 1] #usamos la previa.
                        signo <- -1
                    }else{                           #Caso contrario
                        posSig <-reco$reco[r[3] + 1] #usamos la siguiente.
                        signo <- 1
                    }
                    ...
                    v <- v/normaV    
                    #Procesamos todos los puntos de geolocalizacion de zona.
                    df_prod <- apply(zona,1,function(y){
                        #Vector que une elpunto dee zona con el de recorrido
                        w <- c(y[1],y[2]) - v1
                        prod <- signo*(v[1]*w[1] + v[2]*w[2])
                        prod <- prod/normaV          #Distancia en porcentaje.
                        #Si el producto es positivo se incrementa la distancia.
                        #Hallamos el tiempo total de recorrido aproximado
                        zona$tiempo.dias[y[6]] <<- tiempo[posAct] + 
                            prod*abs(tiempo[posAct] - tiempo[siguiente])
                    })
                    zona$elect <-NULL
                    zona$posicion <- NULL
                    #Evitamos reasignacion de datos.
                    tablaGE$elect[
                        tablaGE$clave %in% zona$clave] <<- rep(TRUE,nrow(zona)) 
                }
                return(zona)
            })
            listaDF[[length(listaDF) + 1]] <- listaRec
        }
        
        #PROGRAMAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAARRRRRRRRRRRRRR
        
        
        objt <- tablaGE$objeto[tiempo <= 200]
        tiempo <- tiempo[tiempo <= 200]
        #Check
        if(length(objt) != length(tiempo)){
            stop("Las longitudes de objt y de tiempo no coinciden.")
        }
        med <- rep("Tiempo en Corriente (Horas)",times=length(objt))
        lat <- rep(x[1],times=length(objt))
        lon <- rep(x[2],times=length(objt))
        datosPtoi <- data.frame(latitude = lat,longitude = lon,objeto = objt,
                                medida = med,valor = tiempo)
        return(datosPtoi)
    }))
}
