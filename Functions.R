...
distGoogleEarth <-function(LatLonTB,tablaGE,dlMax=150,tiempoMax=14){
    ...
    #objeto, medida y valor para cada fila de la tabla LatLonTB. Estos valores 
    ...
    distLin <- unname(apply(LatLonTB,1,function(x){
        ...
        datosPtoi <- data.frame(latitude =lat,longitude =lon,lat.objeto= latObj,
                  lon.objeto = lonObj ,objeto = objt,medida = med,valor = dist2)
    ...
    dataDistLin <- data.frame(latitude = NA,longitude = NA,lat.objeto = NA,
                     lon.objeto = NA,objeto = NA , medida = NA,valor = NA)
    #Procedimiento
    ...
    tiempoRec <- unname(apply(LatLonTB,1,function(x){
        ...
        for(i in 1:6){
            ...
            listaRec <- apply(dfReco,1,function(r){
                ...
                #Contiene los puntos de geolocalizacoin de los objetos.
    ...
    if(length(unique(paste(dataTiempoRec$latitude,dataTiempoRec$longitude))) != 
                                                           nrow(dataTiempoRec)){
