...
CariacoFormat<-function(data,vect,p1=":",p2=":",p3=":",complete=TRUE,nor=TRUE){
    if(nor==TRUE){
        data$x21[11]<-"Lattitude"
        data$x21[data$x1=="Cast 1"]<-df1$lat[1]
        data$x21[data$x1=="Cast 2"]<-df1$lat[2]
        data$x21[data$x1=="Cast 3"]<-df1$lat[3]
        data$x22[11]<-"Longitude"
        data$x22[data$x1=="Cast 1"]<-df1$lon[1]
        data$x22[data$x1=="Cast 2"]<-df1$lon[2]
        data$x22[data$x1=="Cast 3"]<-df1$lon[3] 
    }else{
        data$x23[11]<-"Lattitude"
        data$x23[data$x1=="Cast 1"]<-df1$lat[1]
        data$x23[data$x1=="Cast 2"]<-df1$lat[2]
        data$x23[data$x1=="Cast 3"]<-df1$lat[3]
        data$x24[11]<-"Longitude"
        data$x24[data$x1=="Cast 1"]<-df1$lon[1]
        data$x24[data$x1=="Cast 2"]<-df1$lon[2]
        data$x24[data$x1=="Cast 3"]<-df1$lon[3]
    }

    if(complete==TRUE){
        a<-paste(strsplit(data$x2[vect[1]],split=" ")[[1]][1:3]
                 ,collapse="")
        a<-sub(p1,"",a)
        data$x2[vect[1]]<-a
        b<-paste(strsplit(data$x2[vect[2]],split=" ")[[1]][1:3]
                 ,collapse="")
        b<-sub(p2,"",b)
        data$x2[vect[2]]<-b
        c<-paste(strsplit(data$x2[vect[3]],split=" ")[[1]][1:3]
                 ,collapse="")
        c<-sub(p3,"",c)
        data$x2[vect[3]]<-c
    }
...
#Formateando tablas de Cariaco 2
CariacoFormat2<-function(data,vect,p1=":",p2=":",p3=":",complete=TRUE){
    data$x23[11]<-"Lattitude"
    data$x23[data$x1=="Cast 1"]<-df1$lat[1]
    data$x23[data$x1=="Cast 2"]<-df1$lat[2]
    data$x23[data$x1=="Cast 3"]<-df1$lat[3]
    ...
    data$x24[data$x1=="Cast 3"]<-df1$lon[3]
    if(complete==TRUE){
        a<-paste(strsplit(data$x1[vect[1]],split=" ")[[1]][-c(1,2,6,7,8)]
                 ,collapse="")
        a<-sub(p1,"",a)
        data$x1[vect[1]]<-a
        b<-paste(strsplit(data$x1[vect[2]],split=" ")[[1]][-c(1,2,6,7,8)]
                 ,collapse="")
        b<-sub(p2,"",b)
        data$x1[vect[2]]<-b
        c<-paste(strsplit(data$x1[vect[3]],split=" ")[[1]][-c(1,2,6,7,8,9,10)]
                 ,collapse="")
        c<-sub(p3,"",c)
        data$x1[vect[3]]<-c
    }
