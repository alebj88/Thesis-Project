...
procesamientoDeImagenes<-function(imagPNG,newDir=FALSE,pathPPD="",HT=TRUE){
    ...
    #if(length(unique(clave))-length(numeros)+2 == (length(numeros)-1)){} #Disc
    #if(length(unique(clave))-length(numeros)+2 != (length(numeros)-1)){} #Cont
...
impute.per.region <-function(df1,radio=1,medida="median"){      #Radio en kms.
