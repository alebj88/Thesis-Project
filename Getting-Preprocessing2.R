...
#Formateamos la variable date.
cariaco$date <- unname(sapply(as.character(cariaco$date),function(x){
    d <- strsplit(x,split=" ")[[1]][1]
    return(d)
}))

#Volvemos NA los datos incorrectos.
#Volvemos todas las variables caracter.
for(i in 1:ncol(cariaco)){
    cariaco[,i] <- as.character(cariaco[,i])
}
#Agregamos fila de control
cariaco[nrow(cariaco) + 1,] <- as.character(seq(1:ncol(cariaco)))

#Ejecutamos procedimiento de busqueda.
fun_proc <- apply(cariaco,2,function(var){
    var[var == "lost" | var == "n.d." | var == "n.d"] <- NA
    cariaco[,as.numeric(var[length(var)])] <<- var
})
cariaco <- cariaco[1:(nrow(cariaco) - 1),]

#Asignamos el tipo correcto a cada variable.
for(i in 2:ncol(cariaco)){
    suppressWarnings(cariaco[,i] <- as.numeric(cariaco[,i]))
}

#Corregimos profundidad
cariaco$depth.m <- sapply(cariaco$depth.m,function(dep){
    dep <- round(dep,0)
    return(dep)
})
...
#Damos formato de fecha correcto a boyasNBDC.
boyasNBDC$date <- unname(sapply(boyasNBDC$date,function(x){
    x <- sub("T"," ",as.character(x))
    x <- sub("Z"," ",as.character(x))  
    return(x)
}))

#Las ordenamos antes de exportarlas
cariaco$date <- as.POSIXct(cariaco$date)
cariaco <- arrange(cariaco,date)
boyasArgo$date <- as.POSIXct(boyasArgo$date)
boyasArgo <- arrange(boyasArgo,date)
boyasNBDC$date <- as.POSIXct(boyasNBDC$date)
boyasNBDC <- arrange(boyasNBDC,date)

#Eliminamos clave de world
world$clave2 <- NULL

#Exportamos
setwd("~/Thesis Project AB/Data/Pre-processed Data 2")
write.csv(cariaco,file="cariaco.csv",row.names = FALSE)
write.csv(world,file="WorldOceanographicAtlas.csv",row.names = FALSE)
...
cat("\n")
message("Se agrego a las carpetas 'Pre-processed Data' 1 y 2 el archivo ")
message("'LineaCosteraConCorrientes.csv' exitosamente.\n")

#Creamos las tablas rios y lugares creadas con google earth.
setwd("~/Thesis Project AB/R-Scripts")
source('RiosLugares.R')
message("Se agrego a la carpeta 'Pre-processed Data 2' la tabla:") 
message("'GoogleEarthRiosLugares.csv' de Google Earth.\n")
