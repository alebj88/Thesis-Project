...
#Estudiando filas repetidas.
#tabla<-table(cariacoTS$clave)
#frec<-unname(sapply(cariacoTS$clave,function(x){tabla[x]}))
#repetidos<-cariacoTS[unname(sapply(cariacoTS$clave,function(x){
#    table(cariacoTS$clave)[x]}))!=1,]
#repetidos<-repetidos[order(repetidos$clave),]
#library(tidyr)
#repetidos<-repetidos %>% unique()
