...
setwd("~/Thesis Project AB/Data/Raw Data") #Aqui va a caer la tabla que estamos generando
corrientesCosta <- read.csv("corrientesCostaRaw.csv")  
...
names(LineaCostera)[4] <- "speed.m.s.mean" 
...
LineaCostera <-impute.per.region(LineaCostera,radio = 1,medida="mean")

LineaCostera <-impute.per.region(LineaCostera,radio = 2,medida="mean")

LineaCostera <-impute.per.region(LineaCostera,radio = 5,medida="mean")
...
message("Se agrego a las carpetas 'Getting-Preprocessing' 1 y 2 el archivo 'LineaCosteraConCorrientes.csv' exitosamente.")
