...
# map <- get_map(location= c(lon=-66.715,lat=13.115),zoom=6,maptype="terrain") 
# plotMapa <-ggmap(map) +
#     geom_point(data=corrientesAux,aes(x=longitude,y=latitude,
#                 color=speed.cm.s.mean),show.legend=T) +
#     labs(title="Mapa de Corrientes")	
# plotMapa
...
LineaCostera$speed.cm.s.mean <- rep(NA,times=nrow(LineaCostera))
LineaCostera$direction.deg.t.mean <- rep(NA,times=nrow(LineaCostera))
#Recortamos
corrientesAux <- corrientesAux[corrientesAux$latitude < 13,c(5,9,11,12)]
corrientesAux$posicion <- rep(-1,times=nrow(corrientesAux))
corrientesAux <- corrientesAux[,names(LineaCostera)]
...
LineaCostera <-rbind(LineaCostera,corrientesAux)

LineaCostera <-impute.per.region(LineaCostera,radio = 20,medida="mean")

LineaCostera <-impute.per.region(LineaCostera,radio = 50,medida="mean")
...
#tablaExtra <- LineaCostera
#LineaCostera <- tablaExtra

#Suavizado
LineaCostera$speed.cm.s.mean <- mm(LineaCostera$speed.cm.s.mean)
...
for(i in 1:258){ 
    ...
    angulo <- as.numeric(LineaCostera$direction.deg.t.mean[i+1])/180
    norma <- as.numeric(LineaCostera$speed.cm.s.mean[i+1])
...
LineaCostera$ruta[1] <- "Ruta peninsula"
LineaCostera$speed[1] <- LineaCostera$speed.cm.s.mean[1]
...
map <- get_map(location= c(lon=-66.715,lat=13.115),zoom=6,maptype="terrain")
plotMapa <-ggmap(map) +
    geom_point(data=LineaCostera[1:259,],aes(x=longitude,y=latitude,
                                     color=ruta),show.legend=T) +
...
message("Se agrego a la carpeta 'Getting-Preprocessing' el archivo 'LineaCosteraConCorrientes.csv' exitosamente.")


setwd("~/Thesis Project AB/R-Scripts")
source('RiosLugares.R')
message("Se agregaron a la carpeta 'Getting-Preprocessing 2' las tablas:)
