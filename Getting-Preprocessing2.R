...
#Recuperamos tabla en caso de ser necesario
setwd("~/Thesis Project AB/Data/Pre-processed Data")
...
LineaCostera1 <-impute.per.region(LineaCostera,radio = 1 ,medida="mean")
LineaCostera2 <-impute.per.region(LineaCostera,radio = 2 ,medida="mean")
LineaCostera3 <-impute.per.region(LineaCostera,radio = 5 ,medida="mean")
LineaCostera4 <-impute.per.region(LineaCostera,radio = 10,medida="mean")
LineaCostera5 <-impute.per.region(LineaCostera,radio = 25,medida="mean")
...
margarita1 <-impute.per.region(margarita,radio = 5 ,medida="mean")
margarita2 <-impute.per.region(margarita,radio = 10 ,medida="mean")
margarita3 <-impute.per.region(margarita,radio = 20 ,medida="mean")
...
message("Se agrego a la carpeta 'Pre-processed Data 2' la tabla: 'parques.csv'")
...
message("Se agrego a la carpeta 'Pre-processed Data 2' la tabla: 'DataCorales.csv'")
...
#Data set principal para variable explicada tipo factor.
WOA$clave <- paste(WOA$latitude,WOA$longitude,sep="_")
DataCorales$clave <- paste(DataCorales$latitude,DataCorales$longitude,sep="_")

dataF <- merge(DataCorales,WOA[,-c(1,2)],by="clave",all.x=T)
dataF$clave <- rep(NA,times=nrow(dataF))

WOA$Localidad  <- rep(NA,times=nrow(WOA))
WOA$Especie <- rep(NA,times=nrow(WOA))
WOA$Ubicacion  <- rep(NA,times=nrow(WOA))
WOA$Tipo.de.Arrecife<- rep(NA,times=nrow(WOA))
WOA$Abundancia <- rep(NA,times=nrow(WOA))
WOA$Clase   <- rep(NA,times=nrow(WOA))
WOA <- WOA[,names(dataF)]

dataF  <-rbind(WOA,dataF)
dataF1 <-impute.per.region(dataF,radio = 5 ,medida="mean")
dataF2 <-impute.per.region(dataF,radio = 10 ,medida="mean")
dataF3 <-impute.per.region(dataF,radio = 65 ,medida="mean")

for (i in 2:ncol(dataF)){
    dataF[,i][is.na(dataF[,i])] <- dataF1[,i][is.na(dataF[,i])]
}
for (i in 2:ncol(dataF)){
    dataF[,i][is.na(dataF[,i])] <- dataF2[,i][is.na(dataF[,i])]
}
for (i in 2:ncol(dataF)){
    dataF[,i][is.na(dataF[,i])] <- dataF3[,i][is.na(dataF[,i])]
}
dataF <- dataF[is.na(dataF$clave),]
dataF$clave <- NULL

setwd("~/Thesis Project AB/Data/Pre-processed Data")
write.csv(dataF,file="shinyDataFactor-Var.csv",row.names=F)
