...
if(length(dir()) != 0){
    ...
    if(humanT == TRUE){
        ...
        vars <- names(tablaA)[!(names(tablaA) %in% c(names(tablaB),"depth.m"))]
        ...
        if("depth.m" %in% names(tablaA) == TRUE){
            ...
            if(humanT2 == FALSE){
                ...
                repetir <- TRUE
                while(repetir == TRUE){
                    if(length(prof[!(prof %in% prof[elecciones])]) == 0){
                        message("Seleccion finalizada. Se procesaran los siguientes niveles:")
                        df <- data.frame(I______Profundidades______I = prof[elecciones])
                        print(df)
                        break
                    }
