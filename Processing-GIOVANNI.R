...
if(length(procesar3) != 0){
    if(existe == FALSE){
        cat("Uniendo las series temporales a la matriz ambiental...\n")
        cat("\n")
        matAmb$clave <-paste(matAmb$latitude,matAmb$longitude,sep="_")
        matAmb <-matAmb[,-which(names(matAmb) %in% c("latitude","longitude"))]
        ...
        if(("matAmb" %in% ls()) == FALSE){
            cat("Uniendo las series temporales...\n")
            cat("\n")
            matAmb <-read.csv(procesar3[i])    #VIENEN PREPARADOS PARA EL MERGE.
        ...
        #Si hay datos previamente los unimos.
            ...
            cat("Uniendo las series temporales al resto de los datos...\n")
            cat("\n")
            matAmb$clave <-paste(matAmb$latitude,matAmb$longitude,sep="_")
            matAmb <-matAmb[,-which(names(matAmb) %in% c("latitude","longitude"))]
            for(i in 1:length(procesar3)){
