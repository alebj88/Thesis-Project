...
if(length(procesar2) != 0){
    ...
    for(i in 1:length(procesar2)){     #Procesamos cada carpeta individualmente
        ...
        if(class(df) == "ffdf"){
            ...
            #     elim <- read.csv( ##############################################
            #     "~/Thesis Project AB/Data/Pre-processed Data/clavesExtra1.csv")  
            #     idx <- ffwhich(df,!(medida %in% elim[,1]))        #Comentar esto
            #     df <- df[idx, ]  #Comentar esto#Comentar esto#Comentar esto#Come
            #     elim <- read.csv( ##############################################
            #     "~/Thesis Project AB/Data/Pre-processed Data/clavesExtra2.csv")  
            #     idx <- ffwhich(df,!(medida %in% elim[,1]))        #Comentar esto
            #     df <- df[idx, ]  #Comentar esto#Comentar esto#Comentar esto#Come 
            #     elim <- read.csv( ##############################################
            #     "~/Thesis Project AB/Data/Pre-processed Data/clavesExtra3.csv")  
            #     idx <- ffwhich(df,!(medida %in% elim[,1]))        #Comentar esto 
            #     df <- df[idx, ]  #Comentar esto#Comentar esto#Comentar esto#Come
            #     elim <- read.csv( ##############################################
            #     "~/Thesis Project AB/Data/Pre-processed Data/clavesExtra4.csv")  
            #     idx <- ffwhich(df,!(medida %in% elim[,1]))        #Comentar esto 
            #     df <- df[idx, ]  #Comentar esto#Comentar esto#Comentar esto#Come
            #     #Limpiamos memoria
            #     rm(elim);rm(idx)
