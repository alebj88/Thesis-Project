...
setwd("~/Thesis Project AB/Data/R-Objects")
if("interpretador.RData" %in% dir() == TRUE){
    load("interpretador.RData")           
}else{
    setwd("~/Thesis Project AB/Data/R-Objects/Train Inter")
...
if(length(procesar2) != 0){
    cat("Generando tabla de resumen estadistico para cada serie temporal (ZIP's)...\n")
    cat("\n")
    for(i in 1:length(procesar2)){   #Procesamos cada carpeta individualmente
        ...
        if(length(unique(df$clave)) != nrow(df)){
            df$clave <-as.factor(df$clave)
            d <-as.data.frame(
                tapply(df[,which(names(df) %in% c("medida"))],df$clave,mean))
            d$clave <-row.names(d)
            names(d)[1] <-names(df)[1]
            df <-d
            rm(d)
        ...
        #Compactamos con resumen estadstico.
...
if(actualizo == TRUE){
    ...
    if(humanT == TRUE){
        ...
        message("Para completar la data se pueden emplear la mediana o le media ")
        ...
        if(m %in% c("Media","media","MEDIA")){
            ...
            mat2 <-impute.per.region(mat,radio = r,medida="mean")     #Radio en kmtrs.
        }else{
            message("Usando la mediana...")
            mat2 <-impute.per.region(mat,radio = r,medida="median")
        }
        cat("\n")
