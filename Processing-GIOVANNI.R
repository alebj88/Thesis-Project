...
if(length(procesar)!=0){
    ...
    cat("No hay imagenes individuales en la carpeta 'Giovanni' que procesar.\n")   
    cat("\n")
}
...
if(length(DirectoriosEnPPD) != 0){
    ...
    for (i in 1:length(Directorios)){
        pathGIO <-paste0("~/Thesis Project AB/Data/Raw Data/Giovanni/",Directorios[i])
...
if(length(procesar1)!=0){
    cat("Juntando las tablas GIO que se generaron a partir de imagenes individuales...\n")
    cat("\n")
...
if(length(procesar2) != 0){
    ...
    for(i in 1:length(procesar2)){   #Procesamos cada carpeta individualmente
        setwd(paste0("~/Thesis Project AB/Data/Pre-processed Data/",procesar2[i]))
        ...
        if(nrow(df) > 3000000){     #Creando df$Clave para df's bigdata.
            ...
            if(NroSecc != 0){
                ...
                clv <- data.frame(clave = as.factor(paste(
                    df$latitude[posInf:posSup],df$longitude[posInf:posSup],sep="_")))
                aux1 <-as.ffdf(clv)
        ...
        # print("names/////////////////////////")
        # print(names(df))
        # print("length(unique(df$clave))/////////////////////////")
        # print(length(unique(df$clave)))
        # print("head/////////////////////////")
        # print(head(df))
        # print("sum(is.na(df$medida))/////////////////////////")
        # print(sum(is.na(df$medida)))
...
if(existe == TRUE){
    ...
    if(length(nuevasProcesadas) != 0){
        ...
        #write.csv.ffdf(matAmbOriginal,file="MatrizAmbientalGIO.csv",row.names=F)
...
#actualizo <-TRUE    #Forzando procesamiento
#Imputando matriz ambiental.
if(actualizo == TRUE){
    
    #Le preguntamos al usuario si desea rellenar los datos faltantes de la 
    #matriz ambiental con datos de ubicaciones en un radio proximo.
    message("Indique si desea rellenar los datos faltantes de la matriz ambiental")
    message("usando los datos existentes en entornos proximos.")
    userInput <-FALSE
    while(userInput == FALSE){
        humanT<- readline(prompt="Introduzca Si o No: ")
        cat("\n")
    ...
    #Si desea completar la data solicitamos datos.
    if(humanT == TRUE){
        message("Ingrese el distancia maxima a la que se buscaran datos para")
        message("completar las ubicaciones con valores faltantes.")
        valido <-FALSE
        while(valido == FALSE){
            r <-readline(prompt="Introduzca el valor en kilometros: ")
            cat("\n")
            if(suppressWarnings(is.na(as.numeric(r))) == FALSE){
                if(as.numeric(r) > 0){
                    valido <-TRUE  
                }else{
                    message("Input invalido!")  
        ...
        message("Para completar la data se pueden emplear la mediana o la media ")
        message("ponderada. Indique la medida a implementar.")
        valido0 <-FALSE
        while(valido0 == FALSE){         
            m<- readline(prompt="Introduzca 'mediana' o 'media': ")
            cat("\n")
            if(m %in% c("Media","media","MEDIA","Mediana","mediana","MEDIANA")){
                valido0 <-TRUE
            }else{
                message("Input invalido!")
            }
        ...
        setwd("~/Thesis Project AB/Data/Pre-processed Data 2")
        mat <-read.csv("MatrizAmbientalGIO.csv")  #;mat<-mat[1:100,c(1,2,3,4,5)]

        #Verificacion de tipo de variable.
        claseVars <- unname(sapply(mat,class))
        cat("Para corregir la matriz ambiental las variables deben recibir la \n") 
        cat("clase correcta. Las cualitativas 'factor','character' o 'logical'\n")
        cat("y las cuantitativas 'numeric' o 'integer'. \n")
        cat("\n")
        for(id in 1: ncol(mat)){
            cat("Indique si la clase fue identificada correctamente. \n")
            message("Variable: ",names(mat)[id])
            ...
            if(ip %in% c("si","Si","SI")){
                ...
                if(ip2 %in% c("si","Si","SI")){
                    if(claseVars[id]=="integer" | claseVars[id]=="numeric"){
                        ...
                        mat[,id] <- as.numeric(as.character(mat[,id]))
                    }
        ...
        cat("Corrigiendo los valores faltantes de la matriz ambiental aplicando\n")
        cat("proximidad geografica...\n")
        cat("\n")
        
        #Estableciendo la medida a utilizar
        if(m %in% c("Media","media","MEDIA")){
            message("Usando la media...")
            m <- "mean"     #Radio en kmtrs.
        }else{
            message("Usando la mediana...")
            m <- "median"
        }
        
        #Realizamos el procedimiento variable por variable.
        variables <- names(mat)[!(names(mat) %in% c("latitude","longitude"))]
        
        mat2 <- mat[c("latitude","longitude")]
        f_prod <- sapply(variables,function(v){
            dfVar <- as.data.frame(impute.per.region(
                        mat[c("latitude","longitude",v)],radio = r,medida=m)[v])
            names(dfVar) <- v
            mat2 <<- cbind(mat2,dfVar)
        })

        #Exportamos la tabla.
        cat("\n")
        write.csv(mat2,file="MatrizAmbientalGIO_Imp.csv",row.names = FALSE) 
        message("La matriz ambiental corregida fue creada exitosamente en el directorio Pre-processed Data 2.\n")
    }
}
