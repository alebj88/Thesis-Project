...
if(length(procesar1) != 0){
    ...
    if(length(unique(matAmb$clave)) != nrow(matAmb)){
        ...
        # d <-as.data.frame(tapply(matAmb[,1],matAmb$clave,function(x){
        #     salida <- mean(x,na.rm = TRUE)
        #     return(salida)
        # }))
        # d$clave<-row.names(d)
    ...
    if(length(procesar1) > 1){
        for (i in 2 :length(procesar1)){
            ...
            if(length(unique(df$clave)) != nrow(df)){
                ...
                # d <-as.data.frame(tapply(df[,1],df$clave,function(x){
                #     salida <- mean(x,na.rm = TRUE)
                #     return(salida)
                # }))
                # d$clave <-row.names(d)
...
if(length(procesar2) != 0){
    ...
    for(i in 1:length(procesar2)){     #Procesamos cada carpeta individualmente
        ...
        #Check
        if(any(is.na(df$medida))){
            stop("No se removieron los NA's correctamente.")    
        }
        ...
        setwd("~/Thesis Project AB/Data/Pre-processed Data")
        #Borramos datos manchados solo cuando son de la region establecida
        #por defecto.
        if(alternativo == FALSE){
