...
#Datos de ubicaciones para el merge final
Geo <- Datos[c(1,2,119,120)]
Geo$Clave <- paste(Datos$latitude,Datos$longitude)
Geo$latitude <- NULL
Geo$longitude <- NULL
...
#Merge para hallar localidades localidades
DatosDist$Clave <- paste(DatosDist$latitude,DatosDist$longitude)
DatosTemp$Clave <- paste(DatosTemp$latitude,DatosTemp$longitude)

DatosDist <- merge(Geo,DatosDist,by = "Clave",all.y = TRUE)
DatosTemp <- merge(Geo,DatosTemp,by = "Clave",all.y = TRUE)
