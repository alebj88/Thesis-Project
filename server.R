...
shinyServer(function(input,output){
    ...
    output$RPlot_Map <-renderPlot({
        ...
        #Subsetting para el mapa.
            df <- df[c("latitude","longitude","Localidad")]
    ...
    ACPReact <- reactive({
        ...
        df <- scale(df,scale=FALSE)
        
        #Usamos correlacion o covarianza.
        if(covarianza == TRUE){
            myCors <- cov(df)
        }else{
            if(spearman == TRUE){
    ...
    output$RPlot_C.ACP4 <- renderPlot({
        #Input 
        pca <- ACPReact()
        df <-  AReact()
        ...
        #Construimos manualmente los scores.
        pca$scores <- scale(df,scale=FALSE) %*% pca$loadings 
  
        biplot(pca$scores,pca$scores,main = "Gráfico de Biplot")
        #PCbiplot(pca, colors=c("black", "black", "red", "yellow"))
    })
    ...
    output$RPlot_C.ACP5 <- renderPlot({
        #Input 
        pca <- ACPReact()
        Zvar <- input$Select_ZvarClas
        df <-  AReact()
        ...
        #Eliminamos la variable categorica
        #Construimos manualmente los scores.
        pca$scores <- scale(df[,1:(ncol(df)-1)],scale=FALSE) %*% pca$loadings 
        
        print(dim(pca$scores))
        print(length(df[Zvar][,1]))
        #return(NULL)
        if(input$Box_Numerica2 == TRUE){
            Zvar <- "ZvarFac"
        }
        
        plot(pca$scores[,1:2], pch=20, col=as.factor(df[Zvar][,1]),cex=4)
        #car :: dataEllipse(pca$scores[,1], pca$scores[,2],
        #            as.factor(df[Zvar][,1]), lwd=1,
