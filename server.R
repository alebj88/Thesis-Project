...
shinyServer(function(input,output){
    ...
    output$RUi_Select_Zval <- renderUI({
        ...
        if(input$Box_Numerica == FALSE){
            ...
            alt <- as.character(unique(MatrizBiologica[Zvar][,1])) 
    ...
    output$RUi_Select_Xvar <- renderUI({
        #Genera Select_Xvar
        if(!(input$panels %in% c("Mapa","Diagrama de Cajas","Histogramas"))){
            selectInput("Select_Xvar","Variable de exploración #2:",
                        choices  = as.character(varDepNum)
                        ,
                        selected = as.character(varDepNum)[9]
            )
        }
    })
    ...
    dfLatLonReact <- reactive({
        ...
        withProgress(message = 'Cargando Datos', value = 0, {
            ...
            df <- MatrizBiologica[c("Localidad","latitude","longitude",Zvar,
                           Yvar,Xvar,"Abundancia","lat.objeto.d","lon.objeto.d",
                           "lat.objeto.t","lon.objeto.t")]
            #==========
            incProgress(1/10)
            #==========
    ...
    output$RPlot_Map <-renderPlot({
        ...
        #Subsetting para el mapa.
            df <- MatrizAmbiental[c("latitude","longitude",Yvar)]
    ...
    output$RText_E.Box2 <- renderText({
        c("Resumen Estadístico: Variable ",input$Select_Yvar)
    })
    ...
    SReact <- reactive({
        ...
        if(BoxNum == TRUE){
            ...
            factorVars <- which(names(df) %in% VarDepFactor2)
    ...
    MNmodelReac <- reactive({
        ...
        if(NumBox == TRUE){
            ...
            mymodel <- 
                nnet::multinom(get("ZvarFac") ~ .,data = trg,MaxNWts =100000)
        }else{
            #Check de ingreso
            ...
            mymodel <- 
                nnet::multinom(get(Zvar) ~ .,data = trg,MaxNWts =100000)
        }
    ...
    output$RPrnt_C.ACPEval1 <-renderPrint({ 
        ...
        if(NumBox == TRUE){
            ...
            tab <- table(Predicted = p,Actual = trg["ZvarFac"][,1])
        }else{
            #Check de ingreso
            if("ZvarFac" %in% names(trg) == TRUE){
                return(NULL)
            ...
            p <- predict(mymodel,trg)
            tab <- table(Predicted = p,Actual = trg[Zvar][,1])
        }
    ...
    output$RPrnt_C.ACPEval2 <-renderPrint({ 
        ...
        if(NumBox == TRUE){
            ...
            tab <- table(Predicted = p,Actual = tst["ZvarFac"][,1])
        }else{
            #Check de ingreso
            ...
            p <- predict(mymodel,tst)
            tab <- table(Predicted = p,Actual = tst[Zvar][,1])
        }
    ...
    output$RUi_checkboxGroupACoP <- renderUI({
        ...
        #Render UI.
        checkboxGroupInput("SelectList_ACoPvar","Selector de variables:",
                           choices  = valores
                           ,
                           selected = selected
        )
    ...
    output$RUi_Select_c.Distancia <- renderUI({
        ...
        selectInput("Select_c.Distancia", "Selector de medida:",
    ...
    output$RUi_Select_PCoADim <- renderUI({
        ...
        numericInput("Select_PCoADim","Selector de cantidad:",
                     min = 1,max = maximo,value = 2,step = 1)
    })
    ...
    MatrixReact <- reactive({
        ...
        catVar <- df[names(df)[ncol(df)]]
        df <- df[,-ncol(df)]
        df <- as.matrix(df)
   
        return(list(df,catVar))
    })
    ...
    ACoPReact <- reactive({
        ...
        mds.stuff <- cmdscale(dmatrix,k = dimension ,eig=TRUE)
    ...
    output$RUi_checkboxGroupNMDS <- renderUI({
        ...
        #Render UI.
        checkboxGroupInput("SelectList_NMDSvar","Selector de variables:",
                           choices  = valores
                           ,
                           selected = selected
        )
    ...
    output$RUi_Select_NMDSDim <- renderUI({
        ...
        numericInput("Select_NMDSDim","Selector de cantidad:",
                     min = 1,max = maximo,value = 2,step = 1)
    })
    ...
    #----------------------------------------
    #porcetaje de varianza acumulada
    output$RTable_C.NMDSRes2 <-renderTable({ 
        #Input 
        mds.stuff <- NMDSReact()
        k <- input$Select_NMDSDim
    ...
    #----------------------------------------
    #Autovalores asociados a cada componente.
    output$RTable_C.NMDSRes3 <-renderTable({ 
        #Input 
        mds.stuff <- NMDSReact()
        k <- input$Select_NMDSDim
    ...
    #Gradico de sedimentacion
    output$RPlot_C.NMDSSed <-renderPlot({ 
        #Input 
        mds.stuff <- NMDSReact()
        k <- input$Select_NMDSDim
    ...
    output$RUi_Select_C.NMDSComp1 <- renderUI({
        ...
        numericInput("Select_C.NMDSComp1","Coordenada Principal #1",
                     min = 1,max = maximo,value = 1,step = 1)
    })
    ...
    output$RUi_Select_C.NMDSComp2 <- renderUI({
        ...
        numericInput("Select_C.NMDSComp2","Coordenada Principal #2",
                     min = 1,max = maximo,value = 2,step = 1)
    })
    ...
    output$RUi_checkboxGroup.C.NMDS <- renderUI({
        #Input
        catVar <- as.character(MatrixNMDSReact()[[2]][,1])
    ...
    output$RPlot_C.NMDS <- renderPlot({
        ...
        #Detener plot automatica
        if(input$goButtonANMDS == 0){
            return(NULL)
        }
        #Input 
        mds.stuff <- isolate(NMDSReact())
        Zvar  <- isolate(input$Select_ZvarClas)
        comp1 <- isolate(input$Select_C.NMDSComp1)
        comp2 <- isolate(input$Select_C.NMDSComp2)
        BoxNum <- isolate(input$Box_Numerica2)
        catVar <- isolate(MatrixNMDSReact()[[2]])
